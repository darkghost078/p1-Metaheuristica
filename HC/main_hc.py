from hill_climbing import read_serie, ejecutar_HC, plot_HC_RMSE, plot_final_HC
...
rmse_medios = [
    pd.Series([rep["rmse"] for rep in resultados_HC[i]]).mean() for i in range(4)
]
rmse_desv = [
    pd.Series([rep["rmse"] for rep in resultados_HC[i]]).std() for i in range(4)
]
tiempos_medios = [
    pd.Series([rep["tiempo"] for rep in resultados_HC[i]]).mean() for i in range(4)
]
...
tabla_formateada["Desviación típica"] = tabla_formateada["Desviación típica"].map(
    "{:.6f}".format
)
tabla_formateada["Tiempo medio (s)"] = tabla_formateada["Tiempo medio (s)"].map(
    "{:.6f}".format
)
